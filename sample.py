...
flags.mark_flags_as_required(["workdir", "config", "checkpoint","blur_type"])
...
def main(argv):
    if FLAGS.interpolate:
        sample_interpolate(FLAGS.config, FLAGS.workdir, FLAGS.checkpoint,FLAGS.blur_type,
                           FLAGS.delta, FLAGS.num_points, FLAGS.number)
    else:
        sample(FLAGS.config, FLAGS.workdir, FLAGS.checkpoint,FLAGS.blur_type, FLAGS.save_sample_freq, FLAGS.delta,
               FLAGS.batch_size, FLAGS.share_noise, FLAGS.same_init)


def sample(config, workdir, checkpoint,blur_type, save_sample_freq=1,
...
def sample_interpolate(config, workdir, checkpoint,blur_type,
